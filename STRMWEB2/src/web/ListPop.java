...
public class ListPop extends Shell {
	...
	public static void main(String args[]) {
		try {
			...
			ListPop shell = new ListPop(display,drive,stream,account,in,all);
			shell.open();
			shell.layout();
	 ...
	 * @param display
	 */
	public ListPop(Display display, Driver Drive, Streamer Stream, Account Account, ArrayList<Movie>In, boolean All) {
		...
		int i = 1;
		for(Movie m:in) {
			list.add(i+". "+m.Name()+": "+m.Streams()+" Streams");
			i++;
		}
		...
		lblMostPopularMovies.setText("Most Streamed Movies");
